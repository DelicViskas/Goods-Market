...
export default function GoodCard({ good, toggleFav, session }: { good: Good, toggleFav: () => void, session: Session | null }) {
  ...
  return <Link href={`/good/${id}`} target="_blank" className={`${classes.good} cursor-pointer`}>
    <Slider images={image.length ? image : [placeholderImg]} />
    <span className={classes.title}>{title}</span>
    {session && <ButtonIcon onClick={event => {
      event.stopPropagation();
      event.preventDefault();
      toggleFav();
    }} src={isFavorite ? heartred : heart} height={24} width={24} title="избранное" alt="избранное" />}
